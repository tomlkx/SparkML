...
object demo03 {
  def main(args: Array[String]): Unit = {
    ...
    //自定义UDF　解析xml
    spark.udf.register("xxx", (dataString: String, items: Seq[(String, String)], MachineRecordID: Int, MachineID: Int, MachineRecordState: String, MachineRecordDate: Timestamp) => {
      //解析ｘｍｌ
      ...
      items.foreach(item => {
        //如果找不到补为0.0
        val value: Double = tuples.get(item._1).getOrElse(0.0)
      ...
      val list: Seq[(String, Any)] = Seq(
        ...
        ("machine_record_date", dateFormat.format(new Date())), ("dwd_modify_time", dateFormat.format(MachineRecordDate.getTime))
      )
      list.foreach(line => {
      ...
      mapper.writeValueAsString(buffer)
    })
    ...
    val array = spark.table("MachineData")
      //插入提取列
      .withColumn("itemColumns", typedLit(items))
      //提取所有ColName
      .withColumn("test", expr("xxx(MachineRecordData,itemColumns,MachineRecordID,MachineID,MachineRecordState,MachineRecordDate)"))
      .select("test")
      //对数据进行转换
      .rdd.map(_(0).toString).toDS()
    array.cache()
    ...
    val features: DataFrame = assembler.transform(spark.table("test"))
    //划分数据集合
